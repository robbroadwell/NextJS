...
const Container = styled.section`
    ...
    right: 0;
    bottom: 0;
    display: ${props => props.isRemoveDOM ? "none" : "flex"};
    justify-content: center;
    align-items: center;
    z-index: 5000;
    pointer-events: none;
    transition: opacity 0.3s cubic-bezier(0.25, 0.1, 0.25, 1);
    ${props => props.isLoadingFinished ? "opacity: 0%" : "opacity: 100%"};
`;
...
class Loader extends React.Component {
    ...
    componentDidMount() {
        setTimeout(function() {
            this.setState({ isShowGlobe: true })
        }.bind(this), 200)
    }
  ...
  render() {
      ...
      return (
        <Container isLoadingFinished={this.props.isLoadingFinished} isRemoveDOM={this.props.isRemoveLoaderFromDOM}>
            {this.state.isShowGlobe ? 
            <div>
